...
def softmax(predictions):
    ...
    fun_vectorize = np.vectorize(np.exp) 
    exp_num = fun_vectorize(normalized) 
    sum_exp_num = np.sum(exp_num, axis=1).reshape(exp_num.shape[0],1) 
    softmax_num = exp_num / sum_exp_num 
    return softmax_num
...
def cross_entropy_loss(probs, target_index):
    ...
    prob_class = logfunction_array(probs) 
    real_probability = np.zeros(probs.shape,np.float64) 
    ...
    cross_entropy_losses = np.sum(-prob_class * real_probability) / probs.shape[0] 
...
def linear_softmax(X, W, target_index):
    ...
    predictions = predictions - np.max(predictions, axis = 1).reshape(predictions.shape[0],1)
    exp_vectorize = np.vectorize(np.exp)
    predictions_exp = exp_vectorize(predictions)
    softmax = predictions_exp / np.sum(predictions_exp, axis = 1).reshape(predictions_exp.shape[0],1)
    ln_vectorize = np.vectorize(np.log)
    ln_softmax = ln_vectorize(softmax)
    probability = np.zeros(predictions.shape)
    demention = np.array([i for i in range(target_index.shape[0])]) 
    probability[demention,target_index] = 1 
    loss = np.sum(-probability * ln_softmax) / probability.shape[0]
    dW = np.dot(X.transpose(), softmax - probability) /  probability.shape[0]  
...
class LinearSoftmaxClassifier():
    ...
    def predict(self, X):
        ...
        y_bull = y_classes_numers == np.max(y_classes_numers,axis=1).reshape(X.shape[0],1)
